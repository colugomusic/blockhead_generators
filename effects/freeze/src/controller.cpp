	...
	blink_SR SR)
{
	...
	const auto pitch = (plugin_->env_pitch().envelope().search_vec(data.env_pitch, traverser_->block_positions()) + data.slider_pitch->value) + 60.0f;
	const auto formant = plugin_->env_formant().envelope().search_vec(data.env_formant, traverser_->block_positions());
