...
public:
	...
	ml::DSPVector operator()(int SR, const ml::DSPVector& glottal_output, const ml::DSPVector& fricative_noise);

private:

	void process_transients();
	void add_turbulence_noise(float noise);

	float step(int SR, float glottal_output, float noise, float lambda);
	...
	static constexpr int MOVEMENT_SPEED = 15;

	std::array<float, N> a_;
	std::array<float, N> max_amplitude_;
	...
	std::array<float, NOSE_LENGTH> nose_max_amplitude_;
